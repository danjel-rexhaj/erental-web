...
export default function Favorites({ token, showError, onSelectCar, onSelectCompany, favoriteIds, onToggleFavorite }) {
  ...
  if (visibleCars.length === 0) {
    return (
      <div className="text-center py-16 px-8">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">{t("favorites.emptyTitle")}</p>
        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t("favorites.emptySubtitle")}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-6">{t("nav.favorites")}</h1>
