...
export default function Bookings({ token, showError, showOk, highlightBookingId, refreshKey }) {
  ...
  if (bookings.length === 0) return <div className="text-center py-16 px-8"><Calendar size={28} className="mx-auto text-slate-300 mb-2" /><p className="text-sm text-slate-500">Ende s'ke asnje rezervim.</p></div>;

  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Rezervimet e mia</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          {b.statusi === "pending" && (
            <p className="text-xs text-amber-700 bg-amber-50 rounded-lg px-2 py-1.5 mt-2 flex items-center gap-1">
          ...
          {b.statusi === "completed" && (
            (b.reviews && b.reviews.length > 0) ? (
              <p className="text-xs text-emerald-700 bg-emerald-50 rounded-lg px-2 py-1.5 mt-3">Faleminderit per vleresimin!</p>
            ) : (
...
function ReviewForm({ bookingId, token, onSubmitted, showError }) {
  ...
  return (
    <div className="mt-3 border-t border-slate-100 pt-3">
      <p className="text-xs font-medium text-slate-700 mb-1.5">Lere nje vleresim</p>
      <div className="flex items-center gap-1 mb-2">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} type="button" onMouseEnter={() => setHover(n)} onMouseLeave={() => setHover(0)} onClick={() => setRating(n)}>
            <Star size={18} className={(hover || rating) >= n ? "text-amber-400 fill-amber-400" : "text-slate-200 fill-slate-200"} />
          </button>
        ))}
      </div>
      <textarea
